...
class AuthServiceApis {
  ...
  static Future<void> getAppConfigurations({bool forceConfigSync = false, bool isFromDashboard = false}) async {
    DateTime currentTimeStamp = DateTime.timestamp();
    DateTime lastSyncedTimeStamp = DateTime.fromMillisecondsSinceEpoch(
        getValueFromLocal(SharedPreferenceConst.LAST_APP_CONFIGURATION_CALL_TIME) ?? 0);
    ...
    if (forceConfigSync || currentTimeStamp.isAfter(fiveMinutesLater)) {
      try {
        ...
        setValueToLocal(APICacheConst.APP_CONFIGURATION_RESPONSE, configData.toJson());
      } catch (e) {
        rethrow;
      }
    } else {
