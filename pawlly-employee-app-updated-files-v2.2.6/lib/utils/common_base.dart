...
extension DateData on String {
  ...
  String toFormattedDuration({bool showFullTitleHoursMinutes = false}) {
    try {
      ...
      if (minutes > 0) {
        formattedDuration += '$minutes ${showFullTitleHoursMinutes ? 'minutes' : 'min'}';
      }
...
extension DateExtension on DateTime {
  ...
  String formatTimeMinute() {
    final formatter = DateFormat(DateFormatConst.mm);
    return formatter.format(this);
  }

  String formatTimeHour() {
    final formatter = DateFormat(DateFormatConst.HH_24Hour);
    return formatter.format(this);
  }
...
Color getPriceStatusColor({required BookingDataModel appointment}) {
  if (appointment.payment.paymentStatus.toLowerCase().contains(PaymentStatus.pending)) {
    ...
    return completedStatusColor;
  } else if (appointment.payment.paymentStatus.toLowerCase().contains(PaymentStatus.failed)) {
...
Future<DateTime?> showDurationTimePicker(BuildContext context, {DateTime? selectedDateTime}) async {
  final DateTime? dateTime = await showOmniDateTimePicker(
    ...
    is24HourMode: true,
    isShowSeconds: false,
    theme: ThemeData(dialogTheme: DialogTheme(backgroundColor: context.cardColor)),
    type: OmniDateTimePickerType.time,
    minutesInterval: 1,
    isForce2Digits: true,
    title: Text(locale.value.pickTimeDuration, style: primaryTextStyle(color: primaryColor)).paddingAll(12),
    titleSeparator: const Divider(height: 0),
  );

  return dateTime;
}

String getEmployeeRole({required String userType}) {
  ...
  } else if (userType.contains(EmployeeKeyConst.petStore)) {
    return locale.value.petStore;
  } else {
    return "";
  }
}
...
String getReviewStatus({required String status}) {
  if (status.toLowerCase().contains(ReviewStatus.ByService)) {
    return locale.value.byService;
  } else if (status.toLowerCase().contains(ReviewStatus.ByOrder)) {
    return locale.value.byOrder;
  } else {
    return "";
  }
}
